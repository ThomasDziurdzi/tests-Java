...
public class Jardin {
	...
	public Jardin(final int idJardin, Map<String, Integer> panier2, final IJardinDAO jardinDAO, final IEmplacementDAO emplacementDAO,
	...
	private void init(final int idJardin) {
		...
		for (EmplacementModel emplacementModel : emplacementsModel) {
			try {
				...
				Vegetal veg = (Vegetal) vegClazz.getDeclaredConstructor(Etat.class)
						.newInstance(emplacementModel.etat());
				this.emplacements[emplacementModel.x()][emplacementModel.y()] = new Emplacement(veg);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public void semer() {
		...
		if (x >= 0 && x < jardinModel.longueur() && y >= 0 && y < jardinModel.largeur()
			...
			for (int j = 0; j < jardinModel.longueur(); j++) {
				...
				if (e != null && e.getVegetal().estRecoltable()) {
					...
					if (vegetal instanceof IRacePure) {
						...
						IOgm v = (IOgm) vegetal;
						SimpleEntry<Integer, Integer> nouvellesCoordonnees = v.seDupliquer(jardinModel.longueur(),
								jardinModel.largeur());
						this.emplacements[nouvellesCoordonnees.getKey()][nouvellesCoordonnees
								.getValue()] = new Emplacement(vegetal);
					}
				}
			}
		}
